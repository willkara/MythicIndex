/**
 * Content-only ingestion menu (no imagery)
 */
...
/**
 * Run content-only ingestion (skip imagery)
 */
export async function runContentOnlyMenu(): Promise<void> {
  showSection('Content Only Ingestion');
  ...
  try {
    // Core services (required) - Note: Cloudflare Images NOT needed for content-only
    initD1({
    ...
    if (embeddingsAvailable) {
      succeedSpinner('Services initialized (D1 + semantic search)');
    } else {
      succeedSpinner('D1 initialized (embeddings disabled)');
    }

    // Store for later use
    (config as any).embeddingsAvailable = embeddingsAvailable;
  } catch (err) {
  ...
  try {
    ...
    console.log(chalk.green('  - SKIP all imagery uploads (content only)'));
    ...
    const proceed = await confirm({
      message: 'Proceed with content-only ingestion?',
      default: false,
    });
    ...
    // Run ingestion
    newLine();
    const ingestSpinner = startSpinner('Starting content-only ingestion...');
    ...
    if (result.success) {
      succeedSpinner('Content-only ingestion complete');
    } else {
    ...
    console.log(chalk.dim(`Images: skipped (content-only mode)`));
