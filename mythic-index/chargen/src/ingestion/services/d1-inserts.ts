 ...
 * Returns existing asset ID if hash already exists (deduplication)
 */
 ...
 * Returns existing link ID if same asset+content+role already linked
 */
export async function insertImageLink(link: ImageLinkData): Promise<string> {
  ...
  await execute(
    `INSERT INTO image_link (
			id, asset_id, content_id, scene_id, role, sort_order, caption, alt_text, created_at
		) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      link.id,
      link.assetId,
