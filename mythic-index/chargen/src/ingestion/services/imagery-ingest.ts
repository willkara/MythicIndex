/**
 * Imagery Ingestion Service (new schema only)
 *
 * Parses imagery.yaml files (new standard), uploads images to Cloudflare, and creates DB records.
 */
...
import { query } from './d1-rest.js';
import { uploadImage, computeFileHash, getFileInfo, extractBaseUrl } from './cloudflare-images.js';
import {
  insertImageAsset,
  insertImageLink,
  getImageAssetByHash,
...
import { normalizeImagery, type ImageryKind, type NormalizedImage } from './imagery-normalize.js';
...
/**
 * Ingest imagery.yaml for a chapter, uploading images and creating DB records
 */
...
export async function ingestLocationImagery(
  ...
  for (const img of usableImages) {
    const imgResult = await processImage('location', img, locationDir, contentId, 'scene');
    result.uploaded += imgResult.uploaded;
    result.skipped += imgResult.skipped;
    result.linked += imgResult.linked;
    result.errors.push(...imgResult.errors);
...
/**
 * Ingest all imagery for all chapters with imagery.yaml
 */
...
/**
 * Ingest all imagery for all characters with imagery.yaml
 */
...
/**
 * Ingest all imagery for all locations with imagery.yaml
 */
