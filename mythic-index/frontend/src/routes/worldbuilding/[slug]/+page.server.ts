...
const log = createRouteLogger('/worldbuilding/[slug]');

/**
 * Server load function for worldbuilding entry detail page
 * Loads worldbuilding content with blocks, images, and navigation
 * Supports both new relational schema and legacy unified content table
 * @param params - Route parameters containing worldbuilding slug
 * @param platform - SvelteKit platform with D1 database binding
 * @returns Worldbuilding data with content blocks, images, and prev/next navigation
 * @throws 404 if worldbuilding entry not found
...
export const load: PageServerLoad = async ({ params, platform }) => {
  ...
  log.debug('Loading worldbuilding entry', { action: 'load', slug });

  try {
    const item = await db
      .select()
      .from(contentItem)
      .where(and(eq(contentItem.slug, slug), eq(contentItem.kind, 'worldbuilding')))
      .get();
      ...
      if (revision) {
        ...
        const [prevItem, nextItem] = await Promise.all([
          db
            ...
            .from(contentItem)
            .where(and(eq(contentItem.kind, 'worldbuilding'), lt(contentItem.title, item.title)))
          ...
          db
            .select({ slug: contentItem.slug, title: contentItem.title })
            .from(contentItem)
            .where(and(eq(contentItem.kind, 'worldbuilding'), gt(contentItem.title, item.title)))
            .orderBy(asc(contentItem.title))
            .limit(1)
            .get(),
        ]);
        ...
        return {
          ...
          contentType: 'worldbuilding' as const,
          item: {
            id: item.id,
            title: item.title,
            slug: item.slug,
    ...
    const legacyItem = await db
      .select()
      .from(unifiedContent)
      .where(and(eq(unifiedContent.slug, slug), eq(unifiedContent.type, 'worldbuilding')))
      .get();

    if (!legacyItem) {
      throw error(404, 'Worldbuilding entry not found');
    }
    ...
    const [prevLegacy, nextLegacy] = await Promise.all([
      db
        ...
        .from(unifiedContent)
        .where(
          and(eq(unifiedContent.type, 'worldbuilding'), lt(unifiedContent.title, legacyItem.title))
        )
      ...
      db
        ...
        .from(unifiedContent)
        .where(
          and(eq(unifiedContent.type, 'worldbuilding'), gt(unifiedContent.title, legacyItem.title))
        )
        .orderBy(asc(unifiedContent.title))
        .limit(1)
        .get(),
    ...
    log.error('Failed to load worldbuilding entry', e, { action: 'load', slug, errorMessage });
    throw error(500, `Failed to load worldbuilding entry: ${errorMessage}`);
  }
};
