<script lang="ts">
	...
	async function handleDelete() {
		if (!confirm(`Are you sure you want to delete "${data.character.name}"? This cannot be undone.`)) {
			return;
		}
...
<div class="container mx-auto px-4 py-8 max-w-5xl">
	...
	<div class="mb-6">
		<h1 class="text-3xl font-bold mb-2">Edit Character: {data.character.name}</h1>
		<p class="text-gray-600 dark:text-gray-400">Slug: {slug}</p>
	</div>

	<CharacterForm mode="edit" action="?/update" character={data.character} onCancel={handleCancel} />
</div>
