...
/**
 * Type of content entity for imagery
 */
export type ImageryKind = 'chapter' | 'character' | 'location';

/**
 * Normalized image representation across different YAML schemas
 *
 * Unifies chapter, character, and location imagery formats into a single structure.
 */
...
/**
 * Generation defaults from imagery.yaml metadata
 */
...
/**
 * Generate fallback filename when not explicitly provided
 *
 * @param filePath - File path (may be empty)
 * @param customId - Custom ID (may be undefined)
...
/**
 * Normalize chapter imagery.yaml into common format
 *
 * @param data - Raw chapter imagery YAML data
 * @returns Array of normalized images
 */
...
/**
 * Normalize character imagery.yaml into common format
 *
 * Processes image_inventory with approved images.
 *
 * @param data - Raw character imagery YAML data
 * @returns Array of normalized images
 */
...
/**
 * Normalize location imagery.yaml into common format
 *
 * Processes image_inventory, overview generated images, and zone-specific images.
 *
 * @param data - Raw location imagery YAML data
 * @returns Array of normalized images
 */
export function normalizeLocationImagery(data: any): NormalizedImage[] {
  ...
  if (overviewImages.length) {
	...
	for (const zone of zones) {
		const zoneSlug = zone.slug ?? zone.name ?? 'unnamed';
		const zoneImages = Array.isArray(zone?.generated_images) ? zone.generated_images : [];
		zoneImages
			.filter((img: any) => !img?.archived)
			.forEach((img: any, index: number) => {
				const filePath = img.file_path ?? '';
				...
				images.push({
					...
					tags: Array.isArray(img.tags) ? img.tags : undefined,
					imageType: img.image_type,
					zoneSlug: zoneSlug, // NEW: Extract zone slug
					caption: img.caption ?? img.title,
					alt: img.alt_text ?? img.custom_id ?? fileName,
					quality: img.quality,
					size: img.size,
		...
		sort += zoneImages.length;

		const zoneInventory = Array.isArray(zone?.image_inventory) ? zone.image_inventory : [];
		if (zoneInventory.length) {
			zoneInventory.forEach((item: any, index: number) => {
				...
				if (normalized) {
					normalized.zoneSlug = zoneSlug; // NEW: Add zone slug to inventory images
					images.push(normalized);
				}
			});
			sort += zoneInventory.length;
		}
	}
...
/**
 * Normalize imagery.yaml based on content kind
 *
 * Routes to appropriate normalizer based on entity type.
 *
 * @param kind - Type of content (chapter, character, or location)
 * @param data - Raw imagery YAML data
 * @returns Array of normalized images
 */
